...
class BimanualCatchEnvV1(DirectRLEnv):
    ...
    def __init__(self, cfg: BimanualCatchEnvCfgV1,
        ...
        self.R_closest_dist = torch.zeros((self.num_envs,), device=self.device)
        self.L_closest_dist = torch.zeros((self.num_envs,), device=self.device)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self.R_closest_dist[env_ids] = torch.norm(self.object_pos[env_ids] - self.R_hand_pos[env_ids], dim=-1)
        self.L_closest_dist[env_ids] = torch.norm(self.object_pos[env_ids] - self.L_hand_pos[env_ids], dim=-1)
    ...
    def compute_full_state(self):
        ...
        self.state_buf[:,25:28] = self.L_hand_pos
        self.state_buf[:,28:32] = self.L_hand_rot
        self.state_buf[:,32:50] = self.L_robot_joint_pos
        self.state_buf[:,50:86] = self.actions
        self.state_buf[:,86:89] = self.object_pos
...
def compute_rewards(
    ...
    R_dist_reward = torch.where((R_closest_dist < R_object_dist), torch.tensor(0.0, device=R_closest_dist.device), R_closest_dist - R_object_dist)
    ...
    L_dist_reward = torch.where((L_closest_dist < L_object_dist), torch.tensor(0.0, device=L_closest_dist.device), L_closest_dist - L_object_dist)
    ...
    rewards = (
        ((R_dist_reward_scales * R_dist_reward + L_dist_reward_scales * L_dist_reward) / 2)
